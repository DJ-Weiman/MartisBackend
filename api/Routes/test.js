...
router.get('/getTestsByAssetID', (req, res) => {
	let AssetID = req.query.id.toString();
...
router.post("/orderByLocationAndInspectorAndEmpID", (req, res) => {
  const empLatitude = req.body.empLatitude;
  const empLongitude = req.body.empLongitude;
  const empId = req.body.empId;
  const result = db.orderByLocationAndInspectorAndEmpID(
    empId,
    empLatitude,
    empLongitude
  );
...
router.post("/orderByPriorityAndEmpID", (req, res) => {
  const empId = req.body.empId;
  const result = db.orderByPriorityAndEmpID(empId);
  result
    .then((data) => {
      console.log(data);
      res.json({ data: data });
