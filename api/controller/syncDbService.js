    ...
    try {
      const response = await new Promise((resolve, reject) => {
        for (var x = 0; x < test.length; x++) {
          ...
          var dIssued =
            test[x][1].toString() == "NULL"
              ? null
              : test[x][1].toString().replace(/T/, " ").replace(/\..+/, "");
          var result = test[x][4].toString() == "NULL" ? null : test[x][4];
          var dCompleted =
            test[x][6].toString() == "NULL"
              ? null
              : test[x][6].toString().replace(/T/, " ").replace(/\..+/, "");
  ...
  async importAsset(values) {
    try {
      const response = await new Promise((resolve, reject) => {
        for (var x = 0; x < values.length; x++) {
          //
          const query = `INSERT INTO asset (AssetID, AssetType, Status, GPSLatitude, GPSLongitude, Region, Division, SubDivision, NearestMilePost, LastTestedDate, last_modified) Values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  ...
  async importRole(values) {
    try {
      const response = await new Promise((resolve, reject) => {
        for (var x = 0; x < values.length; x++) {
          //
          const query = `INSERT INTO role (RoleID, Title, CreatedDate, UpdatedDate, DeletedDate, last_modified) Values (?, ?, ?, ?, ?, ?) 
          ...
          var createdD =
            values[x][2].toString() == "NULL"
              ? null
              : values[x][2].toString().replace(/T/, " ").replace(/\..+/, "");
          var updatedD =
            values[x][3].toString() == "NULL"
              ? null
              : values[x][3].toString().replace(/T/, " ").replace(/\..+/, "");
          var deletedD =
            values[x][4].toString() == "NULL"
              ? null
              : values[x][4].toString().replace(/T/, " ").replace(/\..+/, "");
          connection.query(
            query,
            [
              values[x][0],
  ...
  async importUser(values) {
    try {
      const response = await new Promise((resolve, reject) => {
        for (var x = 0; x < values.length; x++) {
          //
          const query = `INSERT INTO user (UserID, Name, Email, Password, Region, RoleID, last_modified) Values (?, ?, ?, ?, ?, ?, ?)
  ...
  async importDevice(values) {
    try {
      const response = await new Promise((resolve, reject) => {
        for (var x = 0; x < values.length; x++) {
          //
          const query = `INSERT INTO device (DeviceID, UserID, PIN, last_modified) Values (?, ?, ?, ?)
  ...
  async importRoleAccess(values) {
    try {
      const response = await new Promise((resolve, reject) => {
        for (var x = 0; x < values.length; x++) {
          //
          const query = `INSERT IGNORE INTO roleaccess (RoleID, AccessID, CreatedDate, UpdatedDate, DeletedDate) Values (?, ?, ?, ?, ?)`;
  ...
  async importRepair(values) {
    try {
      const response = await new Promise((resolve, reject) => {
        for (var x = 0; x < values.length; x++) {
          //
          const query = `INSERT INTO repair (AssetID, CreatedDate, EngineerID, CompletedDate, comments, last_modified) Values (?, ?, ?, ?, ?, ?)
          ...
          var createdD =
            values[x][1].toString() == "NULL"
              ? null
              : values[x][1].toString().replace(/T/, " ").replace(/\..+/, "");
          var completedD =
            values[x][3].toString() == "NULL"
              ? null
              : values[x][3].toString().replace(/T/, " ").replace(/\..+/, "");
